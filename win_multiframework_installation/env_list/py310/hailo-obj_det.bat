...
if exist "%currentDir%\video\hailo_obj_video.mp4" (
    echo Hailo demo video exist.
) else (
    echo Download demo video now.
    curl https://cdn.pixabay.com/video/2016/11/15/6398-191712356_small.mp4?download -o %currentDir%\video\hailo_obj_video.mp4
)
if exist "%currentDir%\model\yolov5m_wo_spp_60p.hef" (
    echo Hailo inference weight exist.
) else (
    echo Download inference weight now.
    curl https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/v2.11.0/hailo8/yolov5m_wo_spp_60p.hef -o %currentDir%\model\yolov5m_wo_spp_60p.hef
)
