...
if dpkg -l | grep hailort-pcie-driver;then
    ...
    yes | sudo dpkg --install $ori_dir/inst/linux/app/driver/hailo/hailort-pcie-driver_4.19.0_all.deb
fi
...
if dpkg -l | grep hailort | grep -v hailort-pcie-driver;then
    ...
    yes | sudo dpkg --install $ori_dir/inst/linux/app/driver/hailo/hailort_4.19.0_amd64.deb
fi
