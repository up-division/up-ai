...
export default function WorkloadForm({ workload }: { workload?: Workload }) {
  ...
  const handleAddWorkload = async () => {
    ...
    try {
      if (
        addWorkload.usecase?.includes('object') &&
      ...
      const isDLStreamer = addWorkload.usecase?.includes('object')
      ...
      toast.success('UP Demo created sucessfully')

      // Redirect to the new workload's page using the ID from the `doc` property
      router.push(`/workload/${response.doc.id}`)
    } catch (error: unknown) {
      const typedError = error as ErrorResponse
      console.error('Failed to create Demo:', typedError.message)
      toast.error(`Failed to add Demo: ${typedError.message}`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleUseCaseChange = (selectedUseCase: UsecaseType) => {
    if (addWorkload.task) {
      setAddWorkload({
        ...
        model: addWorkload.model === 'custom_model' ? 'custom_model' : '',
        devices: [],
        source: selectedUseCase.includes('object')
          ? {
        ...
        metadata: {
          ...(isMetadataObject(addWorkload.metadata)
            ...
            : {}),
          numStreams: selectedUseCase.includes('object') ? 1 : undefined,
        },
      })
    ...
    try {
      ...
      if (typedError.response?.errors) {
        const validationErrors = typedError.response.errors
          ...
          .join(', ')
        toast.error(`Failed to update Demo: ${validationErrors}`)
      } else {
        toast.error(
          `Failed to update Demo: ${typedError.message || 'Unknown error'}`,
        )
      }
    } finally {
      setIsLoading(false)
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle className="text-xl font-bold">
                      {isEdit ? 'Edit' : 'Add'} Demo
                    </CardTitle>
                    <CardDescription>
                      {isEdit ? 'Edit' : 'Add new'} UP Demo
                    </CardDescription>
                  </div>
                  <Button
                    variant="ghost"
              ...
              <CardContent>
                <div className="overflow-hidden">
                  <div className="grid gap-4 py-4">
                    ...
                    {addWorkload.usecase?.includes('object detection') && (
                      <div className="grid gap-2">
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            disabled={addWorkload.usecase?.includes('object')}
                            onChange={(e) => {
                              setModelSelectionType(
                                e.target.value as
                                  | 'predefined'
